...
function ThreadCard({
  ...
  const cardClassName = `thread-sidebar-card relative block rounded-[1.2rem] border px-3 py-2.5 transition ${
    isCurrentThread
...
export function ThreadWorkspaceLayout({
  ...
  function renderNewThreadButton(className: string, compact = false) {
        ...
        <div className="lg:hidden">
          <div className="relative">
            ...
            {showMobileAppMenu && appNavigationMenu && (
              <div className="absolute left-2 top-[calc(100%+0.45rem)] z-20 w-[min(18rem,calc(100vw-1rem))]">
                {appNavigationMenu}
              </div>
            )}
